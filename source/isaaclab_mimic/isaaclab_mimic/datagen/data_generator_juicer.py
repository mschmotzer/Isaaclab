...
class DataGenerator:
    ...
    def __repr__(self):
        ...
        return msg
    def convert_absolute_state_to_relative(self, absolute_state, env_id):
        """Convert absolute state to relative state for the given environment."""
        env_origin = self.env.scene.env_origins[env_id]
        
        # Convert robot poses
        if "articulation" in absolute_state:
            for robot_name, robot_data in absolute_state["articulation"].items():
                if "root_pose" in robot_data:
                    # Convert absolute pose to relative
                    abs_pose = robot_data["root_pose"]
                    rel_pose = abs_pose.clone()
                    rel_pose[:3] -= env_origin[:3]  # Subtract environment origin
                    robot_data["root_pose"] = rel_pose
        
        # Convert object poses
        if "rigid_object" in absolute_state:
            for obj_name, obj_data in absolute_state["rigid_object"].items():
                if "root_pose" in obj_data:
                    abs_pose = obj_data["root_pose"]
                    rel_pose = abs_pose.clone()
                    rel_pose[:3] -= env_origin[:3]  # Subtract environment origin
                    obj_data["root_pose"] = rel_pose
    ...
    def _quat_action_towards(self,current_q_xyzw: np.ndarray, target_q_xyzw: np.ndarray) -> np.ndarray:
        """Delta quaternion such that current ⊗ delta = target."""
        cur = st.Rotation.from_quat(current_q_xyzw)
        tar = st.Rotation.from_quat(target_q_xyzw)
        dq = (cur.inv() * tar).as_quat()
        return dq

    def _stay_quat_action(self,current_q_xyzw: np.ndarray) -> np.ndarray:
        """Delta quaternion that keeps orientation fixed (identity in current frame)."""
        cur = st.Rotation.from_quat(current_q_xyzw)
        dq = (cur.inv() * cur).as_quat()  # identity in quat-delta representation
        return dq
    
    def _pose44_to_pos_quat_np(self,pose44_np: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """pose44_np: shape (4,4). Returns (pos[3], quat_xyzw[4]) as np arrays."""
        R = pose44_np[:3, :3]
        t = pose44_np[:3, 3]
        q_xyzw = st.Rotation.from_matrix(R).as_quat()  # scipy uses [x,y,z,w]
        return t.copy(), q_xyzw.copy()

    def _pose44_torch_to_pos_quat_np(self,pose44_t: torch.Tensor) -> tuple[np.ndarray, np.ndarray]:
        """
        pose44_t can be shape (4,4) or (B,4,4). Returns (pos[3], quat_xyzw[4]) as np arrays.
        Uses the first element if batched.
        """
        if pose44_t.ndim == 3:
            pose = pose44_t[0]
        else:
        ...
        return self._pose44_to_pos_quat_np(pose_np)
    
    def get_state(self,_data, state_index) -> dict | None:
        """Get the state of the specified index from the dataset."""
    
        states = getattr(_data, "states", None)
        ...
        return output_state
    def closest_cube(self, cube_positions: np.ndarray, ee_position: np.ndarray) -> int:
        if len(cube_positions) == 0:
            return -1
        print("ee_position++++++++++++++++++++++++++++++++++++++++++++++", ee_position)
        distances = np.linalg.norm(cube_positions - ee_position, axis=1)
        return np.argmin(distances)

    def check_stacked(self, cube_positions: np.ndarray, cube_velocities: np.ndarray, z_threshold_down: float = 0.04, z_threshold_up: float = 0.048) -> bool:
        if len(cube_positions) == 0:
            return False
        z_values = cube_positions[:, 2]
        cube_speed = np.linalg.norm(cube_velocities[:, :3], axis=1)
        print("CUBE SPEEDS: ", cube_speed)
        moving = np.zeros(len(cube_positions), dtype=bool)
        if len(cube_positions) > 0:
        ...
        for i in range(len(z_diffs)):
            if (z_diffs[i] > z_threshold_down and 
    ...
    """def randomize_bottleneck(self, cur_pos, datagen_info, state_index):
        state = self.get_state(datagen_info, state_index)
        print("STATE INDEX: ", state_index)
        cube_positions = []
        cube_velocities = []
        cube_names = []
        cur_pos = cur_pos.detach().cpu().numpy()
        # Extract cube positions and velocities
        for obj_name, obj_data in state['rigid_object'].items():
            if 'cube' in obj_name.lower() and 'root_pose' in obj_data:
                position = obj_data['root_pose'][:3].detach().cpu().numpy()
                velocity = obj_data['root_velocity'][:3].detach().cpu().numpy()
                cube_positions.append(position)
                cube_velocities.append(velocity)
                cube_names.append(obj_name)

        cube_positions = np.array(cube_positions)

        print("CUBE POSITIONS Before: ", cube_positions)
        cube_positions_mem = cube_positions
        cube_velocities = np.array(cube_velocities)
        print("CUBE VELOCITIES: ", cube_velocities.shape)
        closest_cube_idx = self.closest_cube(cube_positions, cur_pos[:3, -1])
        print("CLOSEST CUBE IDX: ", closest_cube_idx)
        stacked, moving = self.check_stacked(cube_positions=cube_positions, cube_velocities=cube_velocities)
        ...
        while True:
            ...
            while i < len(cube_positions):
                ...
                if (i + 1 < len(cube_positions) and stacked[i] and not moving[i] and not moving[i+1]):
                    # Move both cubes in the stack
                    cube_positions[i][0] = offset_x
                    cube_positions[i][1] = offset_y
                    cube_positions[i + 1][0] = offset_x
                    cube_positions[i + 1][1] = offset_y
                    print("Here")
                    # Clip to environment bounds
                    cube_positions[i][0] = np.clip(cube_positions[i][0], 0.35, 0.7)
                    cube_positions[i][1] = np.clip(cube_positions[i][1], -0.3, 0.3)
                    cube_positions[i + 1][0] = np.clip(cube_positions[i + 1][0], 0.35, 0.7)
                    cube_positions[i + 1][1] = np.clip(cube_positions[i + 1][1], -0.3, 0.3)
                    # Adjust end-effector if closest cube moved
                    i+=1
                    if i == closest_cube_idx:
                        cur_pos[0, -1] = offset_x
                        cur_pos[1, -1] = offset_y
                        cur_pos[0, -1] = np.clip(cur_pos[0, -1], 0.35, 0.7)
                        cur_pos[1, -1] = np.clip(cur_pos[1, -1], -0.3, 0.3)
                        print("Closest cube moved, updated ee position.")

                    
                    if i + 1 < len(cube_positions):
                        if stacked[i]:
                            cube_positions[i + 1][0] = offset_x
                            cube_positions[i + 1][1] = offset_y
                            cube_positions[i + 1][0] = np.clip(cube_positions[i + 1][0], 0.35, 0.7)
                        ...
                        if i == closest_cube_idx:
                            ...
                            cur_pos[1, -1] = np.clip(cur_pos[1, -1], -0.3, 0.3)
                            print("Closest cube moved, updated ee position.")
                    i += 1  # Skip the next cube since it's part of the stack
                    print("Moved stacked cubes at indices", i-1, "and", i)
                elif not moving[i]:
                    # Move single cube
                    cube_positions[i][0] = offset_x
                    cube_positions[i][1] = offset_y
                   ...
                   # print("There")
                    ...
                    i += 1  # Skip this cube
            if self.collide(stacked, cube_positions):
                print("No collisions detected. Final cube positions:", cube_positions)
                break
        return cur_pos,cube_positions, cube_names"""
    def randomize_bottleneck(self, cur_pos, datagen_info, state_index):
        state = self.get_state(datagen_info, state_index)
        #print("STATE INDEX: ", state_index)
        cube_positions = []
        ...
        cube_positions = np.array(cube_positions)
        cubes_moved = np.ones(len(cube_positions), dtype=bool)
        #print("CUBE POSITIONS Before: ", cube_positions)
        cube_positions_mem = cube_positions
        cube_velocities = np.array(cube_velocities)
        #print("CUBE VELOCITIES: ", cube_velocities.shape)
    ...
    async def generate_backward_augment(
        ...
        """
        Create synthetic trajectories by starting from each bottleneck and
        performing a randomized reverse path, then replaying back into the
        bottleneck (no mixing/merging across demos). Each synthetic ep is saved.
        Uses waypoint and queue logic like the main generate().
        """
        ...
        #print("TEST Nr.", len(self.src_demo_datagen_info_pool.datagen_infos))
        number_demos = len(self.src_demo_datagen_info_pool.datagen_infos)
        datagen_info = None
        if number_demos == 1:
            datagen_info = self.src_demo_datagen_info_pool.datagen_infos[0]
        else:
            datagen_info = self.src_demo_datagen_info_pool.datagen_infos[np.random.randint(0, number_demos)]

        bn_indices: List[int] = []
        if "bottlenecks" in datagen_info.subtask_term_signals:
            bn_sig = datagen_info.subtask_term_signals["bottlenecks"]
            if isinstance(bn_sig, np.ndarray) and bn_sig.dtype == bool:
                bn_indices = list(np.where(bn_sig)[0])
            elif isinstance(bn_sig, torch.Tensor):
                if bn_sig.dtype == torch.bool:
                    bn_indices = bn_sig.flatten().nonzero(as_tuple=True)[0].cpu().tolist()
                else:
                    bn_indices = bn_sig.reshape(-1).detach().cpu().numpy().astype(int).tolist()
            else:
                bn_indices = list(map(int, np.array(bn_sig).reshape(-1)))
        bn_t = bn_indices[np.random.randint(0,len(bn_indices))]

        # --- 1. Restore full environment state if available ---
        if hasattr(datagen_info, "states") and datagen_info.states is not None:
            #print("Restoring full state", type(self.get_state(datagen_info,bn_t)))
            try:
                # If you have a full observation dict per timestep, use it
                self.env.reset_to(state=self.get_state(datagen_info,bn_t), env_ids=env_id_tensor)
        ...
        pos_rand = rand_prev_ee_p[:3, 3].cpu().numpy()
        eef_name = next(iter(datagen_info.eef_pose.keys()))
        eef_pose_bn: torch.Tensor = datagen_info.eef_pose[eef_name][bn_t]
        self.grip_bn = float(datagen_info.gripper_action[eef_name][bn_t].item())



        ee_pos_bn, ee_quat_bn = self._pose44_torch_to_pos_quat_np(eef_pose_bn)
        self.grip_bn = float(datagen_info.gripper_action[eef_name][bn_t].item()) if isinstance(
        datagen_info.gripper_action[eef_name], torch.Tensor
        ) else float(datagen_info.gripper_action[eef_name][bn_t])

        grip_tensor = torch.tensor([self.grip_bn], device=self.env.device, dtype=torch.float32)
        trajectory = {}
        ...
        r = np.linalg.norm(pos_rand-ee_pos_bn)
        
        theta = np.deg2rad(np.random.uniform(0,30))
        phi = np.random.uniform( 0,2*np.pi)
        dx, dy, dz = r * np.sin(theta) * np.cos(phi), r * np.sin(theta) * np.sin(phi), r * np.cos(theta)
        rmax = np.deg2rad(0)
        dr, dp, dyaw = np.random.uniform(-rmax, rmax, size=3)
        goal_pos = cur_pos + np.array([dx, dy, dz])
        cur_R = st.Rotation.from_quat(cur_quat)
        d_R = st.Rotation.from_euler("xyz", [dr, dp, dyaw])
        goal_quat = (cur_R * d_R).as_quat()
        n_lin = rand_number_state
        rot_angle = np.linalg.norm(d_R.as_rotvec())
        n_rot = rand_number_state
        lin_path = np.linspace(cur_pos, goal_pos, n_lin)
        slerp = st.Slerp([0, 1], st.Rotation.from_quat([cur_quat, goal_quat]))
        rot_path = slerp(np.linspace(0, 1, n_rot))
        reverse_actions: List[np.ndarray] = []
        #print("Line PPath", lin_path)
        #print("Goal Pos", eef_pose_bn)
        # we want the same number of poses, so interpolate over the longer one
        n_steps = max(n_lin, n_rot)
        #print(n_lin, n_rot, n_steps)
        lin_idx = np.linspace(0, len(lin_path) - 1, n_steps).astype(int)
        rot_idx = np.linspace(0, len(rot_path) - 1, n_steps).astype(int)
    
        

            # Forward movement (not recorded for export)
        for i in range(n_steps):
            pos = lin_path[lin_idx[i]]
            rot = rot_path[rot_idx[i]]
            
            T = np.eye(4, dtype=np.float32)
            T[:3, :3] = rot.as_matrix().astype(np.float32) #np.array([[1,0,0],[0,-1,0],[0,0,-1]])
            T[:3, 3] = pos.astype(np.float32)

            waypoint = Waypoint(
                pose=torch.from_numpy(T).to(self.env.device),
                gripper_action=grip_tensor,
                noise=0.0,
            )
            trajectory[eef_name] = waypoint
            ...
            # Execute but don't save to generated buffers yet
            exec_results = await multi_waypoint.execute(
                env=self.env,
                success_term=success_term,
                env_id=env_id,
            ...
            # Note: Not extending generated_states/obs/actions here

        # RESET RECORDER BEFORE REVERSE MOVEMENT
        print("Resetting recorder before reverse movement...")
        self.env.recorder_manager.reset(env_ids=env_id_tensor)
        self.env.recorder_manager.record_post_reset(env_ids=[env_id])
        # Reset the generated data buffers for the export
        new_initial_state = self.env.scene.get_state(is_relative=True)
        generated_states = []
        generated_obs = []
        generated_actions = []
        ...
        # Reverse movement (this will be recorded and exported)
        break_at_bottleneck = 2
        for i in reversed(range(n_steps)):
            ...
            if i == 0 and break_at_bottleneck>0:
                while True:
                    waypoint = Waypoint(
                        pose=torch.from_numpy(T).to(self.env.device),
                        gripper_action=grip_tensor,
                    ...
                    # NOW record the reverse movement data
                    ...
                    break_at_bottleneck -= 1
                    if break_at_bottleneck <= 0:
                        break
        ...
        # Export only the reverse movement as the episode
