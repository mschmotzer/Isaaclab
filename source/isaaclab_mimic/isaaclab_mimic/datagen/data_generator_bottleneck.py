...
def get_delta_pose_with_scheme(
    ...
    if pos_noise_scale != 0.0 or rot_noise_scale != 0.0:
        pos = delta_pose[:3, 3]
        rot = delta_pose[:3, :3]
        pos_new, rot_new = PoseUtils.add_uniform_noise_to_pose(pos, rot, 0*pos_noise_scale, 0*rot_noise_scale)
        delta_pose = torch.eye(4, device=device)
...
class DataGenerator:
    ...
    def randomize_subtask_boundaries(self) -> dict[str, np.ndarray]:
        ...
        for eef_name, subtask_boundaries in self.src_demo_datagen_info_pool.subtask_boundaries.items():
            ...
            copy_subtask_boundaries = subtask_boundaries.copy()
            ...
            randomized_subtask_boundaries[eef_name] = copy_subtask_boundaries
    ...
    def bottleneck_traj_gen_target(self, src_subtask_target_poses, obj=False):
        # Store original tensor properties
        device = src_subtask_target_poses.device
        dtype = src_subtask_target_poses.dtype

        # Random params (currently fixed values, can be ranges if you like)
        rot_angle = np.random.uniform(-0.5, 0.5)  
        traj_stretch_x = np.random.uniform(1,1)
        traj_stretch_y = np.random.uniform(1, 1)
        ...
        if not obj:
            positions = src_subtask_target_poses[:, :3, 3].detach().cpu().numpy()  # [T,3]
        else:
            positions = src_subtask_target_poses[:3, 3].detach().cpu().numpy()[None, :]  # [1,3]

        # Last point = target, keep fixed
        target = positions[-1]
        deltas = positions - target

        # Transform only deltas
        deltas_transformed = (T @ deltas.T).T
        transformed_positions = target + deltas_transformed

        # Ensure last point stays exactly fixed
        transformed_positions[-1] = target

        # --- Convert back to tensor and assign ---
        transformed_positions_tensor = torch.from_numpy(transformed_positions).to(device=device, dtype=dtype)

        if not obj:
            src_subtask_target_poses[:, :3, 3] = transformed_positions_tensor
        else:
            src_subtask_target_poses[:3, 3] = transformed_positions_tensor[0]

        return src_subtask_target_poses


        """# --- randomized start XY ---
        pos_rand = np.array(
            [np.random.uniform(0.3, 1.5), np.random.uniform(-0.25, 0.25)],
            dtype=np.float32,
        )
        print(pos_rand)
        # --- get container and indices ---
        target_pose_container = src_ep_datagen_info.target_eef_pose[eef_name]  # Use TARGET poses
        start_idx = int(selected_src_subtask_boundary[0])
        end_idx   = int(selected_src_subtask_boundary[-1])

        # --- fetch 4x4 start/end poses, then take translation [:3,3] ---
        if isinstance(target_pose_container, dict):
            k0 = start_idx if start_idx in target_pose_container else str(start_idx)
            k1 = end_idx   if end_idx   in target_pose_container else str(end_idx)
            start_T = _to_numpy(target_pose_container[k0])   # (4,4)
            end_T   = _to_numpy(target_pose_container[k1])   # (4,4)
        else:
            start_T = _to_numpy(target_pose_container[start_idx])  # (4,4)
            end_T   = _to_numpy(target_pose_container[end_idx-1])  # (4,4)

        start_pos = np.array(start_T[:3, 3], dtype=np.float32)  # (3,)
        pos       = np.array(end_T[:3, 3],   dtype=np.float32)  # (3,)

        # --- spline parameterization (ensure >=3) ---
        timesteps = int(max(3, len(selected_src_subtask_boundary)))
        t = np.array([0.0, 0.1 * timesteps, 0.9 * timesteps, float(timesteps)], dtype=np.float32)

        # --- waypoints in XY ---
        delta_xy    = pos[:2] - pos_rand
        waypoint1_xy = pos[:2] + 0.1 * delta_xy
        waypoint2_xy = pos[:2] + 0.9 * delta_xy

        # --- waypoint Z per subtask ---
        if subtask_ind == 0:
            waypoint1_z = float(pos[2] + np.random.uniform(0.05, 0.20))
        elif subtask_ind == 1:
            waypoint1_z = float(np.random.uniform(0.12, 0.14))
        elif subtask_ind == 2:
            waypoint1_z = float(np.random.uniform(0.17, 0.21))
        else:
            waypoint1_z = float(np.random.uniform(0.12, 0.14))

        waypoint2_z = float(pos[2] + 0.05)

        # --- 4 control points (each 3-vector) ---
        p0 = np.array([pos_rand[0],        pos_rand[1],        float(start_pos[2])], dtype=np.float32)
        p1 = np.array([waypoint1_xy[0],    waypoint1_xy[1],    waypoint1_z],        dtype=np.float32)
        p2 = np.array([waypoint2_xy[0],    waypoint2_xy[1],    waypoint2_z],        dtype=np.float32)
        p3 = np.array([float(pos[0]),      float(pos[1]),      float(pos[2])],      dtype=np.float32)
        points = np.stack([p0, p1, p2, p3], axis=0)  # (4,3)

        # --- tangents: vertical at ends, central diff inside ---
        d = np.zeros_like(points, dtype=np.float32)
        d[0]  = np.array([0.0, 0.0, 1.0], dtype=np.float32)
        d[-1] = np.array([0.0, 0.0, 1.0], dtype=np.float32)
        d[1:-1] = (points[2:] - points[:-2]) / (t[2:] - t[:-2])[:, None]

        # --- fit Hermite per axis ---
        spline_x = CubicHermiteSpline(t, points[:, 0], d[:, 0])
        spline_y = CubicHermiteSpline(t, points[:, 1], d[:, 1])
        spline_z = CubicHermiteSpline(t, points[:, 2], d[:, 2])

        # --- sample 0..timesteps (timesteps+1 samples); drop last to align with actions ---
        traj_t  = np.linspace(0.0, float(timesteps), timesteps + 1, dtype=np.float32)
        traj_x  = spline_x(traj_t).astype(np.float32)
        traj_y  = spline_y(traj_t).astype(np.float32)
        traj_z  = spline_z(traj_t).astype(np.float32)
        traj_xyz = np.stack([traj_x, traj_y, traj_z], axis=1)  # (timesteps+1, 3)

        # --- write back into torch tensor [:, :3, 3] ---
        assert isinstance(src_subtask_target_poses, torch.Tensor)
        dev, dtype = src_subtask_target_poses.device, src_subtask_target_poses.dtype
        T = src_subtask_target_poses.shape[0]

        n_fill = min(T, traj_xyz.shape[0] - 1)  # -1: match action count
        traj_slice = torch.from_numpy(traj_xyz[:n_fill].copy()).to(device=dev, dtype=dtype)
        
        src_subtask_target_poses[:n_fill, 0, 3] = traj_slice[:, 0]
        src_subtask_target_poses[:n_fill, 1, 3] = traj_slice[:, 1]
        src_subtask_target_poses[:n_fill, 2, 3] = traj_slice[:, 2]
        
        return src_subtask_target_poses
        #return torch.from_numpy(np.ones([4,4]).copy()).to(device=dev, dtype=dtype)"""
    ...
    def generate_trajectory(
        ...
        #print("shape:", subtask_object_pose)
        ...
        #print("before target poses", src_subtask_target_poses)
        if bottleneck:
            # Apply bottleneck to TARGET poses instead of actual poses
            src_subtask_target_poses = self.bottleneck_traj_gen_target(src_subtask_target_poses)
            #src_subtask_object_pose = self.bottleneck_traj_gen_target(src_subtask_object_pose,obj=True)
            src_subtask_eef_poses = self.bottleneck_traj_gen_target(src_subtask_eef_poses)
            #subtask_object_pose = self.bottleneck_traj_gen_target(subtask_object_pose,obj=True)

        #print("Subtask", subtask_ind, "after bottleneck target poses")
        #   print(src_subtask_target_poses[:, :3, 3])
        ...
        if use_delta_transform is not None:
            ...
            if coord_transform_scheme is not None:
                ...
                #print("Delta Scheme", coord_transform_scheme)
            else:
                if subtask_object_name is not None:
        ...
        if self.env_cfg.datagen_config.generation_interpolate_from_last_target_pose and (not is_first_subtask):
            ...
            init_sequence = WaypointSequence.from_poses(
                ...
                action_noise=0*subtask_configs[subtask_ind].action_noise,
            )
