...
class DataGenerator:
    ...
    def randomize_bottleneck(self, cur_pos, datagen_info, state_index):
        ...
        closest_cube_idx = self.closest_cube(cube_positions, cur_pos[:3, 3])
        ...
        print("MOVING: ", moving)
        print("closest cube index: ", closest_cube_idx)
        while True:
            i = 0
            cube_positions = cube_positions_mem
            while i < len(cube_positions):
                print("i: ", i)
                offset_y = np.random.uniform(-0.3, 0.3)#cube_positions[i][1]
                offset_x = np.random.uniform(0.35, 0.7)#cube_positions[i][0]
    ...
    async def generate_backward_augment(
        ...
        new_initial_state = self.env.scene.get_state(is_relative=False)
        generated_states = []
        generated_obs = []
        generated_actions = []
        ...
        if hasattr(datagen_info, "states") and datagen_info.states is not None:
            ...
            try:
                ...
                state=self.get_state(datagen_info,bn_t)
                self.env.reset_to(state=state, env_ids=env_id_tensor, is_relative=False)
            except Exception as e:
                print(f"[BA] Could not restore full state from observation: {e}")
        rand_number_state=np.random.randint(42,46)
        grip= None
        half = int(np.floor(rand_number_state/3*2))
        halfplus = int(half/2)
        ...
        if bn_t-half >= 0:
            ee_traj= datagen_info.eef_pose[next(iter(datagen_info.eef_pose.keys()))][bn_t-half:bn_t+halfplus+1].clone()
            grip = datagen_info.gripper_action[eef_name][bn_t-half:bn_t+halfplus+1].clone()
            state=self.get_state(datagen_info,bn_t+halfplus)
        else:
            ee_traj = datagen_info.eef_pose[next(iter(datagen_info.eef_pose.keys()))][0:rand_number_state+1].clone()
            grip = datagen_info.gripper_action[eef_name][0:rand_number_state+1].clone()
            state=self.get_state(datagen_info,rand_number_state)
        cubes_z_term = torch.tensor([], dtype=torch.float32, device=self.env.device)
        
        for obj_name, obj_data in state['rigid_object'].items():
            if 'cube' in obj_name.lower() and 'root_pose' in obj_data:
                z_value = obj_data['root_pose'][2]
                #print("CUBE Z AT BN: ", z_value)
                if z_value.ndim == 0:  # if it's a scalar
                    z_value = z_value.unsqueeze(0)
                cubes_z_term = torch.cat([cubes_z_term, z_value])
        self.env._cube_z_pos = cubes_z_term  # Store as list

        eef_pose_bn: torch.Tensor = ee_traj[half,:,:].clone()
        grip_bn: torch.Tensor = grip[half,:].clone()
        ...
        eef_pose_bn = torch.tensor(eef_pose_bn[:3, 3], device=ee_traj.device, dtype=ee_traj.dtype)
        ee_traj[:,:3, 3] -=  eef_pose_bn
        eef_pose_bn = cur_T

        for i in range(50):
            waypoint = Waypoint(
                    pose=torch.from_numpy(cur_T).to(self.env.device),
                    gripper_action=grip_bn,
                    noise=0.0,
                )
        ...
        state = self.env.scene.get_state(is_relative=False)
        for i, obj_name in enumerate(cube_names):
            if cubes_moved[i]:
                ...
                old_pose[0,3:] = torch.tensor(np.array([0,0,0,1]), dtype=old_pose.dtype, device=old_pose.device)
        ...
        rot_matrix = st.Rotation.from_euler('z', rot_angle).as_matrix()

        # Only rotate the translational component, keep orientation unchanged
        ee_traj_np = ee_traj.detach().cpu().numpy()  # Convert to numpy
        cur_T_np = cur_T if isinstance(cur_T, np.ndarray) else cur_T.detach().cpu().numpy()
        
        positions = ee_traj_np[:, :3, 3]  # Extract positions from numpy array
        rotated_positions = np.matmul(rot_matrix, positions.T).T  # Apply rotation
        # Create new trajectory preserving orientations but updating positions
        bn_trajectory = ee_traj_np.copy()
        bn_trajectory[:, :3, 3] = rotated_positions + cur_T_np[:3, 3]  # Update all positions at once

        for i in reversed(range(int(np.floor(len(bn_trajectory))/2))):
            
            T = bn_trajectory[i]
            #print(f"Reversing waypoint {i}, target z: {T[2,3]}")
            waypoint = Waypoint(
                pose=torch.from_numpy(T).to(self.env.device),
                gripper_action=grip[i],
                noise=0.0,
            )
            trajectory[eef_name] = waypoint
            multi_waypoint = MultiWaypoint(trajectory)
        ...
        generated_actions = []
        generated_success = False   
        safe_prev_z = cur_T[2, 3]
        for i in range(len(bn_trajectory)):
            #print(f"Previous waypoin: {safe_prev_z} current ee pos: {self.env.get_robot_eef_pose(env_ids=[env_id], eef_name=eef_name)[0][2,3]}")
            
            T = bn_trajectory[i]
            """if i != 0:
                current_ee_pos = self.env.get_robot_eef_pose(env_ids=[env_id], eef_name=eef_name)[0][:3, 3]
                T[2, 3] = T[2, 3] - 0.0000 * (current_ee_pos[2] - safe_prev_z)"""
